(function () {
    ...
    function pick(obj, candidates) {
        for (var i = 0; i < candidates.length; i++) {
            if (
                obj[candidates[i]] !== undefined &&
                obj[candidates[i]] !== null
            ) {
    ...
    function render(smiles) {
        ...
        try {
            ...
            var rings = pick(descriptors, [
                "NumRings",
                "ringCount",
                "NumAliphaticRings",
            ]);
            var hbd = pick(descriptors, [
                "NumHBD",
                "lipinskiHBD",
                "NumHDonors",
            ]);
            var hba = pick(descriptors, [
                "NumHBA",
                "lipinskiHBA",
                "NumHAcceptors",
            ]);
    ...
    function init() {
        ...
        if (typeof window.initRDKitModule !== "function") {
            setStatus(
                "Não foi possível carregar o RDKit.js (verifique a conexão).",
            );
            return;
        }
